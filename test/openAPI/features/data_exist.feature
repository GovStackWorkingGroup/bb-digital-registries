...
Feature: API endpoint that allows users to check if an entry exists in the database.
  ...
  @smoke @positive @unit
    Scenario: Successfully receives a message that the record exists in database smoke type test
      ...
      When send POST request to check if the record exist in the database is sent with given Information-Mediator-Client header and path params "registryname" as registryname and "111" as versionnumber
